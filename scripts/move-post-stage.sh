...
if [[ "$to" == "post" ]]; then
  ...
  moved_index="$to_dir/$slug/index.md"
  if [[ "$from" == "draft" && "$to" == "queue" && -f "$moved_index" ]]; then
    # If a draft used explicit /drafts/<slug>/ URLs, retarget them for queue previews.
    perl -0pi -e "s#/drafts/$slug/#/queue/$slug/#g" "$moved_index"
    echo "Updated /drafts/$slug/... links to /queue/$slug/... in $moved_index"
  fi
